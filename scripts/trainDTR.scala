...
import org.apache.spark.mllib.evaluation.BinaryClassificationMetrics
  ...
  def labeledPoints(action: String, position: Position, locals: Array[String], jobcates: Array[String], actionCount: Map[String, Int]) =
  {
...
//  }
//
def labeledPoint(values: Array[String], locals: Array[String], jobcates: Array[String]) =
{
    val p = position(values)
    val features = Vectors.dense(p.lrFeatures(locals, jobcates))
...
.split(", ").toSet
val userLocals = Source.fromFile(new File("data/user_locals")).getLines().map(_.split("\001")).filter(values => cookies.contains(values(0))).map(values => (values(0), values(1).split(";"))).toMap
...
val datas = rawdatas.flatMap { case (action, position) => labeledPoints(action, position, locals, jobcates, actionCount) }.cache
//datas.map(lp => (lp.label,1)).reduceByKey(_+_).foreach(println)

val model = DecisionTree.trainRegressor(datas, scala.collection.immutable.Map[Int,Int](), impurity="variance", maxDepth=10, maxBins=40) // 9:0.6552971213620207 10:0.6653916818137815  0.6660773324726795

val result = testData.map(_.split("\t")).map(values => labeledPoint(values,locals,jobcates)).map(lp => (model.predict(lp.features), lp.label))
