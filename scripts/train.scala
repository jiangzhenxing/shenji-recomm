...
val dtmodel = DecisionTree.trainRegressor(datas, scala.collection.immutable.Map[Int, Int](), impurity="variance", maxDepth=9, maxBins=32) // 9:0.6448457311761356

val result = testData.map(_.split("\t")).map(labeledPoint).map(lp => ((logistic(vecdot(lrmodel.weights.toArray, lp.features.toArray))) * 10 + dtmodel.predict(lp.features) * base, lp.label))
