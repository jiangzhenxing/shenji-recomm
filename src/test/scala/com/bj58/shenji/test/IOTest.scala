...
//    var scate3 = Source.fromFile(new File("data/scate3_result")).getLines().mkString(";")
//    println(scate3)
    
//    val userLocals = Source.fromFile(new File("data/user_locals")).getLines().map(_.split("\001")).map(values => (values(0), values(1).split(";"))).toMap
//    println(userLocals)
//    val userJobCates = Source.fromFile(new File("data/user_job_cates")).getLines().map(_.split("\001")).map(values => (values(0), values(1).split(";"))).toMap
//    println(userJobCates)
    Source.fromFile(new File("data/cmc_local")).getLines().filter(_.split("\t").size < 2).foreach(println)
  }
}